...
def plot2D(solution_vector, basis_vector, translation_vector):
	...
	if solution_vector is not None and basis_vector is not None and translation_vector is not None:
		...
		if not np.all(np.isinf(solution_vector)):
			...
			# Unique Solution
			ax.scatter(u[0], u[1], color='red', scale_units='width',
			...
			ax.quiver(0, 0, u[0], u[1], scale_units='width', label=f'Basis Vector ({round(u[0], 2)}, {round(u[1], 2)})',
					  color="g")
			ax.quiver(0, 0, p[0], p[1], scale_units='width',
